// script.js 파일 전체 코드 - Firebase Realtime Database 및 Authentication 사용, QR 스캐너 통합
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ====================================================================
    // Firebase Authentication (Google 로그인) 관련 함수 (선택 사항)
    // 이 앱에서는 학번/반/번호 입력 방식이므로, Google 로그인은 필수는 아닙니다.
    ...
    async function startQrScanner() {
        // 스캐너가 이미 실행 중인 경우 중지 후 다시 시작하여 카메라 리소스 해제 및 재할당
        if (html5QrcodeScanner && html5QrcodeScanner.isScanning) {
            try {
                await html5QrcodeScanner.stop();
                console.log("QR 스캐너 중지됨.");
            } catch (error) {
                console.warn("QR 스캐너 중지 실패 (이미 중지되었을 수 있음):", error);
            }
        }

        // Html5QrcodeScanner 인스턴스 생성
        // "qr-video"는 비디오 스트림이 표시될 HTML 요소의 ID입니다.
        html5QrcodeScanner = new Html5QrcodeScanner(
            "qr-video", 
            { 
                fps: 10, // 초당 프레임 수 (스캔 속도)
                qrbox: { width: 250, height: 250 }, // QR 코드 스캔 박스 크기 (UI)
                // 카메라 선택 옵션 (모바일에서 후면 카메라 우선)
                facingMode: "environment" 
            },
            /* verbose= */ false // 자세한 로그 출력 여부 (개발 시 true로 설정하면 디버깅에 도움)
        );

        // QR 스캔 성공 시 호출될 콜백 함수
        const onScanSuccess = async (decodedText, decodedResult) => {
            console.log(`QR 스캔 성공: ${decodedText}`, decodedResult);
            qrResult.textContent = `스캔 성공: ${decodedText}`;

            // 스캐너 중지 (한 번 스캔하면 카메라 리소스를 해제하여 배터리 소모 방지)
            try {
                await html5QrcodeScanner.stop();
                console.log("QR 스캐너 중지됨.");
            } catch (error) {
                console.error("QR 스캐너 중지 중 오류 발생:", error);
            }

            // 스캔된 텍스트(clubId)를 숫자로 변환
            const scannedClubId = Number(decodedText);

            // 유효한 동아리 ID인지 확인 (1부터 15 사이)
            if (scannedClubId >= 1 && scannedClubId <= 15) {
                // 현재 학생이 로그인되어 있는지 확인
                if (!currentStudent) {
                ...
                // 이미 스탬프를 찍었는지 확인
                if (currentStampedClubs.includes(scannedClubId)) {
                    alert('이미 스탬프를 찍은 동아리입니다.');
                } else {
                    // 스탬프 추가 및 Firebase에 저장
                    currentStampedClubs.push(scannedClubId);
                    await saveStampsFirebase({ // Firebase 함수 호출
                        ...currentStudent,
                    ...
                    const clubName = clubDataList.find(c => c.id === scannedClubId)?.name || '해당 동아리';
                    alert(`${clubName} 스탬프가 찍혔습니다!`);
                    updateStampImages(); // 스탬프 이미지 UI 업데이트
                    updateStudentInfoDisplay(); // 학생 정보 표시 업데이트
                    updateTenStampsMessage(); // 10개 스탬프 메시지 업데이트
                }
            } else {
                alert('유효하지 않은 QR 코드입니다. 동아리 스탬프 QR 코드를 스캔해주세요.');
            }

            // 스캔 후 다시 스캐너 시작 (선택 사항: 계속 스캔하게 하려면 이 주석을 해제)
            // startQrScanner();
        };

        // QR 스캔 오류 시 호출될 콜백 함수
        const onScanError = (errorMessage) => {
            // console.warn(`QR 스캔 오류: ${errorMessage}`); // 너무 많은 오류 메시지 방지
            // qrResult.textContent = `스캔 오류: ${errorMessage}`; // UI에 오류 메시지 표시 (선택 사항)
        };

        // 스캐너 시작
        console.log("QR 스캐너 시작 시도 중...");
        html5QrcodeScanner.render(onScanSuccess, onScanError); // 스캐너 렌더링 시작
    }
    ...
    let clubDataList = []; // 동아리 목록 데이터 (Firebase에서 로드)
    ...
    function showScreen(screenElement) {
        ...
        if (screenElement) {
            screenElement.style.display = 'flex'; // 화면을 보이게 함 (CSS에서 flex로 설정되어 있어야 함)
            console.log(`화면: ${screenElement.id}`);
        } else {
            console.error("유효하지 않은 화면 요소입니다.");
        }

        // 화면 전환 시 QR 스캐너 제어 로직
        if (screenElement === mainContent) {
            // 메인 화면으로 전환될 때만 QR 스캐너 시작
            startQrScanner(); 
        } else {
            // 다른 화면으로 전환될 때 스캐너 중지 (카메라 리소스 해제)
            if (html5QrcodeScanner && html5QrcodeScanner.isScanning) {
                html5QrcodeScanner.stop().catch(err => console.warn("QR 스캐너 중지 중 오류:", err));
            }
        }
    }
    ...
    submitInfoBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            updateTenStampsMessage(); // 10개 스탬프 메시지 업데이트
            showScreen(mainContent); // 메인 화면으로 전환 (이때 QR 스캐너 시작)
    ...
    // 각 스탬프 이미지 클릭 이벤트 리스너 설정 (QR 스캔으로만 스탬프 찍도록 변경)
    for (let i = 1; i <= 15; i++) {
        const stampImg = stampElements[`stamp-${i}`];
        if (stampImg) {
            stampImg.addEventListener('click', async () => {
                // 스탬프 이미지를 직접 클릭하는 것을 방지하고 QR 스캔을 유도
                alert('스탬프는 QR 코드를 스캔하여 찍을 수 있습니다.'); 
                return; 
            });
        }
    }
